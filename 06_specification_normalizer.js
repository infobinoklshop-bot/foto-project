...
const NORMALIZER_FUNCTIONS = {
  ...
  normalizeRange: (value) => {
    ...
    // Ищем паттерн: [знак]число [любые символы] тире/пробел [любые символы] [знак]число
    const match = str.match(/(-?\d+(?:[.,]\d+)?)\s*[^\d\-]*?[-–—]\s*[^\d\-]*?(\+?-?\d+(?:[.,]\d+)?)/);
    if (match) {
      let first = match[1].replace(',', '.');
      let second = match[2].replace(',', '.');
      // Нормализуем формат: убираем лишние плюсы, форматируем
